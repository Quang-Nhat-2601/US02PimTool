...
var ProjectFormManagement = /** @class */ (function () {
    function ProjectFormManagement(executionContext) {
        this.executionContext = executionContext;
        this.project = null;
        this.mapStatusToNumber = {
            "New": "283630000",
            "Planned": "283630001",
    ...
    ProjectFormManagement.prototype.initializeContext = function () {
        var _a, _b, _c;
        if (this.executionContext) {
            this.context = this.executionContext.getFormContext();
        }
        else if (typeof window.GetGlobalContext === "function") {
            this.context = window.GetGlobalContext();
        }
        else if (typeof ((_a = Xrm === null || Xrm === void 0 ? void 0 : Xrm.Utility) === null || _a === void 0 ? void 0 : _a.getGlobalContext) === "function") {
            this.context = Xrm.Utility.getGlobalContext();
        }
        else if (typeof ((_c = (_b = parent.Xrm) === null || _b === void 0 ? void 0 : _b.Utility) === null || _c === void 0 ? void 0 : _c.getGlobalContext) === "function") {
            this.context = parent.Xrm.Utility.getGlobalContext();
        }
        else {
            console.warn("GetGlobalContext is not available, falling back to window.Xrm");
            this.context = window.Xrm || parent.Xrm;
        }
        if (!this.context) {
            throw new Error("Unable to initialize Dynamics 365 context");
        }
    };
    ProjectFormManagement.prototype.getClientUrl = function () {
        var _a, _b;
        return ((_b = (_a = this.context).getClientUrl) === null || _b === void 0 ? void 0 : _b.call(_a)) || '';
    };
    ProjectFormManagement.prototype.getXrm = function () {
    ...
    ProjectFormManagement.prototype.getDataParam = function () {
        if (window.location.search.indexOf('Data=') === -1)
            return null;
        var data = decodeURIComponent(window.location.search.split('Data=')[1]);
        return JSON.parse(data);
    };
    ProjectFormManagement.prototype.init = function () {
        var dataParam = this.getDataParam();
        console.log("Params: ".concat(JSON.stringify(dataParam)));
        if (dataParam && dataParam.entity && dataParam.entityId) {
            console.log("Entity: ".concat(dataParam.entity));
            console.log("entityId: ".concat(dataParam.entityId));
    ...
    ProjectFormManagement.prototype.loadProjectData = function (entity, entityId) {
        var xrm = this.getXrm();
        xrm.WebApi.retrieveRecord("account", "6dc4fed6-fa52-ef11-9195-0050569f8865", "?$select=accountid").then(function (result) {
    ...
    ProjectFormManagement.prototype.populateForm = function () {
        if (!this.project)
            return;
        jquery_default()('#projectNumber').val(this.project.elca_projectnumber);
        jquery_default()('#projectName').val(this.project.elca_name);
        jquery_default()('#customer').val(this.project.elca_customer);
        jquery_default()('#group').val(this.project.elca_projectgroupid);
        jquery_default()('#members').val(this.project.elca_members);
        jquery_default()('#status').val(this.project.elca_projectstatus);
        jquery_default()('#startDate').val(this.project.elca_startdate);
        jquery_default()('#endDate').val(this.project.elca_enddate);
        // Update button text
        jquery_default()('#create-btn').text('Update Project');
    };
    ProjectFormManagement.prototype.updateProject = function () {
        if (!this.project)
            return;
        var updatedProject = {
            elca_projectnumber: jquery_default()('#projectNumber').val(),
            elca_name: jquery_default()('#projectName').val(),
            elca_customer: jquery_default()('#customer').val(),
            elca_projectgroupid: jquery_default()('#group').val(),
            elca_members: jquery_default()('#members').val(),
            elca_projectstatus: this.mapStatusToNumber[jquery_default()('#status').val()],
            elca_startdate: jquery_default()('#startDate').val(),
            elca_enddate: jquery_default()('#endDate').val(),
        };
        var xrm = this.getXrm();
        xrm.WebApi.updateRecord("elca_project", this.project.elca_projectid, updatedProject).then(function (result) {
    ...
    ProjectFormManagement.prototype.bindEvents = function () {
        var _this = this;
        jquery_default()('#create-btn').on('click', function () { return _this.updateProject(); });
        // Add more event bindings as needed
    };
    return ProjectFormManagement;
}());
function onXrmReady(callback) {
    if (window.Xrm) {
        callback();
    }
    else {
        setTimeout(function () { return onXrmReady(callback); }, 100);
    }
}
// Initialize the app when Xrm is ready
onXrmReady(function () {
    var app = new ProjectFormManagement();
    app.bindEvents();
});
